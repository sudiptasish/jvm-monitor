...
public class UISelectionPanel extends JPanel implements MouseListener {
	...
	private _UI_SELECTIONPANEL_GRAPHPANEL_SHARED sharedObj = null;
	...
	public UISelectionPanel(String name, _UI_SELECTIONPANEL_GRAPHPANEL_SHARED obj) {
		panelName = name;
		sharedObj = obj;
		setSize(dim);
	...
	private void addComponent() {
		...
		if (panelName.equals(PANEL_TYPE_MEMORYPOOL)) {
			selections = MBeanSharedObjectRepository.getInstance().getMpool_mx_bean().getMemoryPoolNames();
		}
		else if (panelName.equals(PANEL_TYPE_MEMORY)) {
			selections = MBeanSharedObjectRepository.getInstance().getMemory_mx_bean().getMemoryNames();
		}
		else if (panelName.equals(PANEL_TYPE_GBCOLLECTOR)) {
			selections = MBeanSharedObjectRepository.getInstance().getGbcollector_mx_bean().getGarbageCollectorNames();
		}
		else if (panelName.equals(PANEL_TYPE_THREAD)) {
			selections = new String[] {"Main Thread"};
		}
		else if (panelName.equals(PANEL_TYPE_CLASS)) {
			selections = MBeanSharedObjectRepository.getInstance().getClass_mx_bean().getAllClassTypes();
		}
